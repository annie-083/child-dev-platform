...
export default function PostDetail() {
  ...
  async function loadAll() {
    ...
    if (authorIds.length > 0) {
      ...
      namesById = Object.fromEntries((profiles || []).map((p) => [p.user_id, p.display_name]))
    }
    setComments(rawComments.map((c) => ({ ...c, authorName: namesById[c.author_id] || 'ผู้ปกครอง' })))
        ...
        <>
          ...
          {comments.map((c) => (
            <div key={c.id} style={{ marginBottom: 12, borderBottom: '1px solid var(--color-line)', paddingBottom: 10 }}>
              <p style={{ fontSize: 12, fontWeight: 600, margin: '0 0 3px' }}>{c.authorName}</p>
