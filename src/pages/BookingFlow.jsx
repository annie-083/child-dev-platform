...
export default function BookingFlow() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data: childData } = await supabase.from('children').select('id, nickname')
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 6, marginBottom: 16 }}>
            {uniqueDates.length === 0 && (
              <p style={{ fontSize: 13, color: 'var(--color-ink-soft)' }}>ไม่มีคิวว่างในช่วงนี้</p>
            )}
            {uniqueDates.map((date) => (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  whiteSpace: 'nowrap',
                  background: selectedDate === date ? 'var(--color-forest)' : 'var(--color-panel)',
                  color: selectedDate === date ? '#fff' : 'var(--color-ink-soft)',
                  boxShadow: selectedDate === date ? 'none' : 'inset 0 0 0 1px var(--color-line)',
                }}
              >
